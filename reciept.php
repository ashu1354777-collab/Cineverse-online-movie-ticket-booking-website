...
<body>
	...
	<div>
		...
		$ORDERID_PARAM = mysqli_real_escape_string($con, $_GET['id']);
		$qry = "SELECT b.*, m.movieTitle FROM bookingtable b LEFT JOIN movietable m ON b.movieID=m.movieID WHERE b.ORDERID='" . $ORDERID_PARAM . "'";
		...
		$amount = $row['amount'];
		$ORDERID = $row['ORDERID'];
		$createdAt = $row['DATE-TIME'];

		$qrText = "Order: $ORDERID | $movieTitle | $bookingDate $time | $bookingFName $bookingLName";
		ob_start();
		QRcode::png($qrText, null, QR_ECLEVEL_L, 4, 2);
		$qrImg = base64_encode(ob_get_clean());
		?>
	</div>
	<br>
	<div class="invoice-box">
			...
			<tr class="top">
				<td colspan="2">
					<table>
						...
						Created: <?php date_default_timezone_set('Asia/Kolkata'); echo DATE("d-m-y h:i:s", strtotime($createdAt)); ?><br>
		...
		<?php
		echo '<div style="text-align:center;padding:16px;"><h4>E-Ticket QR Code</h4><img src="data:image/png;base64,' . $qrImg . '" alt="QR Code" style="max-width:220px;" /></div>';
		?>
